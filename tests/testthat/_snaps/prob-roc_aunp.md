...
# `options` is defunct
    ...
    Code
      roc_aunp(two_class_example, truth, Class1, Class2, options = 1)
    Condition
      Error:
      ! The `options` argument of `roc_aunp()` was deprecated in yardstick 1.0.0 and is now defunct.
      i This argument no longer has any effect, and is being ignored. Use the pROC package directly if you need these features.

---

    Code
      roc_aunp_vec(truth = two_class_example$truth, estimate = as.matrix(
        two_class_example[c("Class1", "Class2")]), options = 1)
    Condition
      Error:
      ! The `options` argument of `roc_aunp_vec()` was deprecated in yardstick 1.0.0 and is now defunct.
      i This argument no longer has any effect, and is being ignored. Use the pROC package directly if you need these features.
