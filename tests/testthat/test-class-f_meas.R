test_that("Calculations are correct - two class", {
  ...
  expect_equal(
    f_meas_vec(truth = df_2_1$truth, estimate = df_2_1$prediction),
    0.5882353,
    tolerance = 0.0001
  )
})
...
test_that("Calculations are correct - three class", {
  ...
  expect_equal(
    f_meas(multi_ex, estimator = "micro")[[".estimate"]],
    0.6
  )
})
...
test_that("All interfaces gives the same results", {
  ...
  exp <- f_meas_vec(pathology$pathology, pathology$scan)

  expect_identical(
    f_meas(path_tbl)[[".estimate"]],
    exp
  )
  expect_identical(
    f_meas(path_mat)[[".estimate"]],
    exp
  )
  expect_identical(
    f_meas(pathology, truth = pathology, estimate = scan)[[".estimate"]],
    exp
  )
})
...
test_that("`NA` values propagate from binary `precision()` (#77)", {
...
test_that("`NA` values propagate from binary `recall()` (#77)", {
