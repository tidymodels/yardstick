...
#'  If a full set of class probability columns are passed to `...`,
#'  then there is also a column for [mnLogLoss()]. When `truth` has
#'  two levels and there are class probabilities, [roc_auc()] is
#'  appended. When `truth` is numeric, there are columns for
#'  [rmse()] and [rsq()],
